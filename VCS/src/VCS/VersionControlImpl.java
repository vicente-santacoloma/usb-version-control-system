 ...
 * @author Guille
 */
public class VersionControlImpl extends UnicastRemoteObject implements VersionControl {
    ...
    for (int i = 0; i < files.length; i++) {
      if (!totalFiles.contains(files[i].getFileName())) {
        for (int k = 0; k < _kTolerance; k++) {
          ...
          descrip[0] = new FileDescription(files[i].getFileName(), files[i].getVersion(), files[i].getTimestamp(), files[i].getUserName(), files[i].getData());
  ...
  public synchronized Object[] checkout()
    ...
    try {
      ...
      pack = new DatagramPacket(bSend, bSend.length, InetAddress.getByName("225.0.0.5"), 41895);
  ...
  @Override
   public Object[] updateServer(int id)
    ...
    for (Element file : FileParser.getDataElements(server)) {
      ...
      for (Object fo : files) {
        FileDescription f = (FileDescription) fo;
  ...
  public synchronized boolean requestEntry(int id, InetAddress ip)
    ...
    for (Element s : FileParser.serverList(config)) {
      System.out.println(FileParser.getValueOfServer(s, "id"));
      if (Integer.parseInt(FileParser.getValueOfServer(s, "id")) == id) {
        exists = true;
        break;
      }
